    ...
    return {
      ...
      publish: function (models, eventName, context, data, req) {
        ...
        if (!models) {
          ...
          sails.log.silly('Published ', eventName, ' to ', self.classRoom(req));
          self.broadcast( self.classRoom(req), eventName, data, socketToOmit );
          return;
        }
       ...
       * @api private
       */
      _classRoom: function(req) {
        var tail = '';
        if(req) {
          var me = this;
          var subrooms = Object.keys(me.subrooms).sort();
          _.each(subrooms, function(subroom) {
            if(me.subrooms[subroom]) { // take only subrooms with value true or similar
              tail += ':'+req.body[subroom];
            }
          });
        }
        return 'sails_model_create_'+this.identity+tail;
      },
      ...
      watchers: function(req) {
        return sails.sockets.subscribers(this._classRoom(req), true);
      },
      ...
      */
      ...
      publishCreate: function(values, req, options) {
        ...
        this.broadcast(this._classRoom(req), eventName, {
          verb: 'created',
          data: values,
          id: values[this.primaryKey]
        }, socketToOmit);
      ...
      watch: function ( req ) {
        ...
        sails.sockets.join(socket, this._classRoom(req));
        sails.log.silly("Subscribed socket ", sails.sockets.id(socket), "to", this._classRoom(req));

        // if (sails.config.sockets['backwardsCompatibilityFor0.9SocketClients'] && socket.handshake) {
        //   var sdk = app.getSDKMetadata(socket.handshake);
        //   var isLegacySocketClient = sdk.version === '0.9.0';
        //   if (isLegacySocketClient) {
        //     sails.sockets.join(socket, this._classRoom(req)+':legacy_v0.9');
        //   }
        // }
      ...
      unwatch: function ( req ) {
        ...
        sails.sockets.leave(socket, this._classRoom(req));
        sails.log.silly("Unubscribed socket ", sails.sockets.id(socket), "from", this._classRoom(req));

        // if (sails.config.sockets['backwardsCompatibilityFor0.9SocketClients'] && socket.handshake) {
        //   var sdk = app.getSDKMetadata(socket.handshake);
        //   var isLegacySocketClient = sdk.version === '0.9.0';
        //   if (isLegacySocketClient) {
        //     sails.sockets.leave(socket, this._classRoom(req)+':legacy_v0.9');
        //   }
        // }
      },
      ...
      introduce: function(model, req) {

        var id = model[this.primaryKey] || model;

        _.each(this.watchers(req), function(socket) {
